...
int app_protocol_message_to_json(const message_t *msg, char *out_buf, size_t out_buf_size) {
    ...
    if (json_add_string(root, "target", msg->target.id) != 0) {
        goto cleanup;
    }
    if (json_add_string(root, "sender", msg->sender.id) != 0) {
        goto cleanup;
    }
    ...
    if (json_add_string(root, "message_id", msg->message_id.id) != 0) {
        goto cleanup;
    }
    if (json_add_string(root, "transaction_id", msg->transaction_id.id) != 0) {
        goto cleanup;
    }
    if (json_add_u32(root, "message_type", msg->message_type) != 0) {
        goto cleanup;
    }
    if (json_add_string(root, "ack_for_message_id", msg->ack_for_message_id.id) != 0) {
        goto cleanup;
    }
...
int app_protocol_message_from_json(const uint8_t *buffer, size_t buffer_size, message_t *out_msg)
{
    ...
    if (json_read_string_field(root, "target",
                               temp_msg.target.id,
                               sizeof(temp_msg.target.id)) != 0) {
        goto cleanup;
    }
    if (json_read_string_field(root, "sender",
                               temp_msg.sender.id,
                               sizeof(temp_msg.sender.id)) != 0) {
        goto cleanup;
    }
    ...
    if (json_read_string_field(root, "message_id",
                               temp_msg.message_id.id,
                               sizeof(temp_msg.message_id.id)) != 0) {
        goto cleanup;
    }
    if (json_read_string_field(root, "transaction_id",
                               temp_msg.transaction_id.id,
                               sizeof(temp_msg.transaction_id.id)) != 0) {
        goto cleanup;
    }
    ...
    if (json_read_string_field(root, "ack_for_message_id",
                               temp_msg.ack_for_message_id.id,
                               sizeof(temp_msg.ack_for_message_id.id)) != 0) {
        goto cleanup;
    }
